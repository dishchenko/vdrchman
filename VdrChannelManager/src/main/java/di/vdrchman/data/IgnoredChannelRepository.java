...
public class IgnoredChannelRepository {
	 ...
	 *            COMPARISON_CHANGED_IGNORED_FORCED - changed channels (compared
	 *            to the channels from scanned channel list) eligible for forced
		...
		case COMPARISON_NOT_SCANNED:
			if (transpId < 0) {
				if (sourceId < 0) {
					query = em
							.createQuery(
									"select ic from IgnoredChannel ic, Transponder o_t, Source o_s where ic.transpId = o_t.id and o_t.sourceId = o_s.id and o_s.userId = :userId and not exists (select sc from Source s, Transponder t, ScannedChannel sc where ic.transpId = t.id and t.sourceId = s.id and sc.sourceName = s.name and sc.frequency = t.frequency and sc.polarization = t.polarization and sc.streamId = t.streamId and sc.sid = ic.sid and sc.apid = ic.apid and sc.userId = :userId) order by ic.id",
									IgnoredChannel.class);
					query.setParameter("userId", sessionUser.getId());
				} else {
					query = em
			...
			break;
		case COMPARISON_CHANGED_IGNORED_FORCED:
			if (transpId < 0) {
				if (sourceId < 0) {
					query = em
							.createQuery(
									"select ic from IgnoredChannel ic, Transponder o_t, Source o_s where ic.transpId = o_t.id and o_t.sourceId = o_s.id and o_s.userId = :userId and exists (select sc from Source s, Transponder t, ScannedChannel sc where ic.transpId = t.id and t.sourceId = s.id and sc.sourceName = s.name and sc.frequency = t.frequency and sc.polarization = t.polarization and sc.streamId = t.streamId and sc.sid = ic.sid and sc.apid = ic.apid and (coalesce(ic.providerName, ' ') <> coalesce(sc.providerName, ' ')) and sc.userId = :userId) order by ic.id",
									IgnoredChannel.class);
					query.setParameter("userId", sessionUser.getId());
				} else {
					query = em
	...
	/**
	 * Adds the channel to the persisted list of ignored channels (stores it in
	 * the database).
	 * 
	 * @param channel
	...
	/**
	 * Updates the channel in the persisted list of ignored channels (updates it
	 * in the database).
	 * 
	 * @param channel
	 *            the ignored channel to update
	 */
	public void update(IgnoredChannel channel) {
		em.merge(channel);
	}
